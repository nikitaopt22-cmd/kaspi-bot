...
def choose_format(message):
    ...
    bot.send_message(message.chat.id, "Выбери формат:", reply_markup=markup)
...
def callback_format(call):
    ...
    bot.send_message(call.message.chat.id, "Пришли ZIP или PDF!")

@bot.message_handler(content_types=['document'])
def handle_doc(message):
    print(f"Получен файл от {message.from_user.id}: {message.document.file_name}")
